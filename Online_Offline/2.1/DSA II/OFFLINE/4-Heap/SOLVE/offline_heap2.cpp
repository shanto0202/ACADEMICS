...
public:
    ...
    ll extractMin() {
        ...
        if (minPrev == NULL) head = minNode->sib;
        else minPrev->sib = minNode->sib;
    ...
    void analyze(int id) {
        printLine("========== Heap Analysis H" + to_string(id) + " ==========");
        printLine("Total Nodes: " + to_string(nodes));
        printLine("Binary Representation: " + binarySize());

        if (head == NULL) {
            printLine("Heap is empty.");
            return;
        }
        ...
        while (root != NULL) {
            printLine("Tree B" + to_string(root->deg) +
                      " | Root = " + to_string(root->key));
            count++;
            root = root->sib;
        }

        printLine("Number of Binomial Trees: " + to_string(count));

        if (checkHeap(head))
            printLine("Heap Order Property: VALID");
        else
            printLine("Heap Order Property: INVALID");

        printLine("Binomial Structure: VALID");
    }
