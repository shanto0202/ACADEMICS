#include <bits/stdc++.h>
#include <iomanip> // প্রিন্ট ফরম্যাটিংয়ের জন্য যোগ করা হয়েছে
using namespace std;
...
class ChainHash
{
    ...
    void checkGrow()
    {
        if (loadFactor() > MAX_LOAD && insertedAfterGrow >= lastGrowCount / 2)
        {
            ...
            // The first growth establishes the starting point for deletion counting.
...
// ---------------- Double Hashing and Custom Probing -----------------
    ...
    void insert(const K &key, const V &value)
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            else
            {
                ...
                // Count one collision only when two distinct keys have the
                // same primary hash value, as required in the assignment.
...
int main()
{
    ...
    // The performance report specifically requires words of length 10.
    const int wordLength = 10;

    vector<string> words = generateWords(wordLength);
    vector<int> searchIndex(TO_SEARCHES);
    for (int &index : searchIndex) index = rand() % TOTAL_KEYS;
    ...
    // নতুন প্রিন্ট ফরম্যাট যোগ করা হয়েছে 
