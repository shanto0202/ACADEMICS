...
template <typename K>
int auxiliaryHash(const K &key, int tableSize)
{
...
class ChainHash
{
    ...
    void checkShrink()
    {
        if (tableSize > INIT_SIZE && loadFactor() < MIN_LOAD &&
            deletedAfterShrink >= lastShrinkCount / 2)
        {
...
public:
    ChainHash(int choice = 1)
    {
        ...
        hashChoice = choice;
        collisions = 0;
        insertedAfterGrow = deletedAfterShrink = 0;
        lastGrowCount = lastShrinkCount = 0;
...
class OpenAddressHashTable
{
    ...
    double loadFactor() { return (double)elementCount / tableSize; }
    ...
    int probeIndex(const K &key, int i)
    {
        ...
        if (!customProbing) return (h1 + i * h2) % tableSize;
...
public:
    OpenAddressHashTable(int choice, bool useCustom)
    {
        tableSize = INIT_SIZE;
        elementCount = 0;
        hashChoice = choice;
        customProbing = useCustom;
        collisions = 0;
        insertedAfterGrow = deletedAfterShrink = 0;
        lastGrowCount = lastShrinkCount = 0;
        table.resize(tableSize);
    ...
    void insert(const K &key, const V &value)
    {
        ...
        for (int i = 0; i < tableSize; i++)
        {
            ...
            else
            {
                ...
                if (primaryCount[home] > 0) collisions++;
                primaryCount[home]++;
...
int main()
{
    ...
    // র্যান্ডম নাম্বার জেনারেটর সিড করা হলো যেন প্রতিবার আলাদা আলাদা ডেটাসেট তৈরি হয়
    srand(time(0)); 

    const int wordLength = 10;

    // ৩টি মেথডের জন্য সম্পূর্ণ আলাদা ৩টি ডেটাসেট জেনারেট করা হলো
    vector<string> wordsChaining = generateWords(wordLength);
    vector<string> wordsDouble = generateWords(wordLength);
    vector<string> wordsCustom = generateWords(wordLength);

    // ৩টি মেথডের জন্য আলাদা আলাদা সার্চ ইন্ডেক্স লিস্ট
    ...
    for (int hashChoice = 1; hashChoice <= 2; hashChoice++)
    {
        ...
        // প্রতিটা মেথডে তার নিজস্ব আলাদা ডেটাসেট পাস করা হচ্ছে
        ...
        pair<long long, double> c = testTable(customProbing, wordsCustom, searchCustom);

        collision[0][hashChoice - 1] = a.first; hits[0][hashChoice - 1] = a.second;
        collision[1][hashChoice - 1] = b.first; hits[1][hashChoice - 1] = b.second;
        collision[2][hashChoice - 1] = c.first; hits[2][hashChoice - 1] = c.second;
    }
    ...
    for (int i = 0; i < 3; i++)
        cout << left << setw(20) << method[i] 
             << setw(20) << collision[i][0] 
             << setw(15) << hits[i][0] 
             << setw(20) << collision[i][1] 
             << setw(15) << hits[i][1] << '\n';
