...
unsigned long long makeHash(const K &key, int base)
{
    ...
    for (char ch : text) hashVal = hashVal * base + ch;
    return hashVal;
}
...
class ProbeHash
{
    ...
    int probeIndex(const K &key, int i)
    {
        ...
        if (!customHash) return (h1 + i * h2) % Size;
    ...
    void resizeTable(int newSize)
    {
        ...
        while (true)
        {
            ...
            for (auto &block : oldTable)
            {
                if (block.state == 1 && !place(block.key, block.value))
                {
                    successful = false;
                    break;
                }
            ...
            if (successful)
            {
                for (auto &block : oldTable)
                    if (block.state == 1)
                        primaryCount[primaryHash(block.key, Size, hashNo)]++;
                return;
            }
...
public:
    ProbeHash(int choice, bool useCustom)
    {
        Size = INIT_SIZE;
        elements = 0;
        hashNo = choice;
        customHash = useCustom;
        collisions = 0;
...
int main()
{
    ...
    vector<int> searchDouble(TO_SEARCHES), searchCustom(TO_SEARCHES);
    ...
    long long collision[2][2];
    double hits[2][2];

    for (int hashNo = 1; hashNo <= 2; hashNo++)
    {
        ProbeHash<string, int> doubleHashing(hashNo, false);
        ProbeHash<string, int> customProbing(hashNo, true);

        auto a = testTable(doubleHashing, wordsDouble, searchDouble);
        auto b = testTable(customProbing, wordsCustom, searchCustom);
    ...
    string method[2] = {"Double Hashing", "Custom Probing"};
    ...
    for (int i = 0; i < 2; i++)
        cout << left << setw(20) << method[i]
             << setw(20) << collision[i][0] << setw(15) << hits[i][0]
             << setw(20) << collision[i][1] << setw(15) << hits[i][1] << '\n';
